...
pub enum StringError {
  ...
  OutOfBoundsLength,
  OutOfBoundsEncoding,
  VarIntParseError(&'static str),
}

impl ToString for StringError {
  fn to_string(&self) -> std::string::String {
    match self {
      StringError::InvalidUTF8 => "Invalid UTF-8 string".to_string(),
      StringError::InvalidLength => "Invalid string length".to_string(),
      StringError::OutOfBoundsLength => "String is too long".to_string(),
      StringError::OutOfBoundsEncoding => "String encoding is too long".to_string(),
      StringError::VarIntParseError(err) => format!("Failed to parse VarInt: {err}"),
    }
  }
}
...
impl String {
  ...
  pub fn to_bytes(&mut self) -> Result<Vec<u8>, StringError> {
    #[allow(clippy::cast_possible_truncation)]
    #[allow(clippy::cast_possible_wrap)]
    let len = self.data.len().try_into().map_err(|_| StringError::OutOfBoundsLength)?;
    let mut varint_bytes = VarInt::new(len).encode().map_err(StringError::VarIntParseError)?;
  ...
  pub fn from_bytes(bytes: &[u8]) -> Result<Self, StringError> {
    let mut position = 0;
    let mut size = VarInt::decode(&bytes[position..]).map_err(StringError::VarIntParseError)?;

    position += size.encode().map_err(StringError::VarIntParseError)?.len();
...
mod test {
  ...
  fn invalid_utf8_bytes_001() {
    ...
    assert_eq!(result, Err(StringError::InvalidUTF8));
  }
  ...
  fn invalid_length_001() {
    ...
    let result = String::from_bytes(&invalid_bytes);
    assert_eq!(result, Err(StringError::InvalidLength));
  }

  #[test]
  fn invalid_length_002() {
    // The real string length is 55, but here we say it's 123
    let invalid_bytes = [vec![0x7B], b"Traversing the mountain ranges to seek hidden treasures".into()].concat();
    let result = String::from_bytes(&invalid_bytes);
    assert_eq!(result, Err(StringError::InvalidLength));
  }
  ...
  fn invalid_length_003() {
    ...
    let invalid_bytes = [vec![0xA1], b"The symphony of nature orchestrates a mesmerizing dance".into()].concat();
    let result = String::from_bytes(&invalid_bytes);
    assert_eq!(result, Err(StringError::InvalidLength));
  }
  ...
  fn invalid_length_005() {
    ...
    let invalid_bytes = [vec![0x92], b"Lost in the maze of thoughts, seeking clarity and truth".into()].concat();
    let result = String::from_bytes(&invalid_bytes);
    assert_eq!(result, Err(StringError::InvalidLength));
  }
