...
impl VarInt {
  ...
  pub fn decode(buf: &[u8]) -> Result<VarInt, &'static str> {
    let mut value = 0;
    let mut position = 0;
  ...
  pub fn encode(&mut self) -> Result<Vec<u8>, &'static str> {
    ...
    loop {
      ...
      self.0 &= !(!0 << (32 - 7)); // Masking to ensure zero-fill behavior
    }
  }
...
mod test {
  ...
  fn read_0() {
    assert_eq!(
      VarInt::decode(&[0x00]).unwrap().value(),
      0
    );
  }
  ...
  fn read_1() {
    assert_eq!(
      VarInt::decode(&[0x01]).unwrap().value(),
      1
    );
  }
  ...
  fn read_2() {
    assert_eq!(
      VarInt::decode(&[0x02]).unwrap().value(),
      2
    );
  }
  ...
  fn read_127() {
    assert_eq!(
      VarInt::decode(&[0x7f]).unwrap().value(),
      127
    );
  }
  ...
  fn read_128() {
    assert_eq!(
      VarInt::decode(&[0x80, 0x01]).unwrap().value(),
      128
    );
  }
  ...
  fn read_255() {
    assert_eq!(
      VarInt::decode(&[0xff, 0x01]).unwrap().value(),
      255
    );
  }
  ...
  fn read_25565() {
    assert_eq!(
      VarInt::decode(&[0xdd, 0xc7, 0x01]).unwrap().value(),
      25565
    );
  }
  ...
  fn write_2097151() {
    assert_eq!(
      VarInt(2_097_151).encode().unwrap(),
      vec![0xff, 0xff, 0x7f]
    );
  }
  ...
  fn read_2147483647() {
    assert_eq!(
      VarInt::decode(&[0xff, 0xff, 0xff, 0xff, 0x07]).unwrap().value(),
      2_147_483_647
    );
  }
  ...
  fn read_minus_1() {
    assert_eq!(
      VarInt::decode(&[0xff, 0xff, 0xff, 0xff, 0x0f]).unwrap().value(),
      -1
    );
  }
  ...
  fn read_minus_2147483648() {
    assert_eq!(
      VarInt::decode(&[0x80, 0x80, 0x80, 0x80, 0x08]).unwrap().value(),
      -2_147_483_648
    );
  }
